...
export function stackedBars(data, series, opts = {}) {
  ...
  data.forEach((d, i) => {
    ...
    series.forEach((s, si) => {
      ...
      bars += `<rect x="${x(i).toFixed(1)}" y="${y1.toFixed(1)}" width="${bw.toFixed(1)}" height="${(y0 - y1).toFixed(1)}" fill="${PALETTE[si % PALETTE.length]}" rx="1.5"><title>${esc(d.label || d.x || d.date)} ${esc(s.name)} ${esc(vtxt)}</title></rect>`;
    });
  });
  // 客数の折れ線（右軸）
  let line = '';
