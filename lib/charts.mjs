...
export function stackedBars(data, series, opts = {}) {
  ...
  data.forEach((d, i) => {
    ...
    series.forEach((s, si) => {
      ...
      bars += `<rect x="${x(i).toFixed(1)}" y="${y1.toFixed(1)}" width="${bw.toFixed(1)}" height="${(y0 - y1).toFixed(1)}" fill="${PALETTE[si % PALETTE.length]}" rx="1.5"><title>${esc(d.label || d.date)} ${esc(s.name)} ${yen(v)}</title></rect>`;
    });
  });
  // 客数の折れ線（右軸）
  let line = '';
  ...
  // グリッド＋Y軸
  let grid = '';
  for (let g = 0; g <= 4; g++) { const v = (ymax / 4) * g, yy = y(v); grid += `<line x1="${pad.l}" y1="${yy.toFixed(1)}" x2="${W - pad.r}" y2="${yy.toFixed(1)}" stroke="var(--grid)" stroke-width="1"/><text x="${pad.l - 6}" y="${(yy + 3).toFixed(1)}" text-anchor="end" class="ax">${(v / 1000).toFixed(0)}k</text>`; }
  let xlab = '';
  data.forEach((d, i) => { if (n > 16 && i % 2) return; xlab += `<text x="${(x(i) + bw / 2).toFixed(1)}" y="${H - 12}" text-anchor="middle" class="ax">${esc((d.date || '').slice(5))}</text>`; });
...
export function donut(rows, opts = {}) {
  ...
  rows.forEach((row, i) => {
    ...
    seg += `<path d="M ${p(a0, r)} A ${r} ${r} 0 ${large} 1 ${p(a1, r)} L ${p(a1, r - thick)} A ${r - thick} ${r - thick} 0 ${large} 0 ${p(a0, r - thick)} Z" fill="${PALETTE[i % PALETTE.length]}"><title>${esc(row.label)} ${yen(row.value)} (${(frac * 100).toFixed(0)}%)</title></path>`;
    a0 = a1;
  });
  return `<svg viewBox="0 0 ${S} ${S}" class="donut" role="img">${seg}<text x="${cx}" y="${cy - 2}" text-anchor="middle" class="dcenter">${yen(total)}</text><text x="${cx}" y="${cy + 16}" text-anchor="middle" class="dsub">総売上</text></svg>`;
}
