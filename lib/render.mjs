...
function hourlySection(scope) {
  return section('時間帯別の入場者数（平均）', '入場時刻ベース・1営業日あたり平均人数', vBars(scope.hourly.map((h) => ({ label: h.h + '時', value: h.avg })), { color: 'var(--c3)', kfmt: (v) => String(Math.round(v * 10) / 10), vfmt: (v) => v + '人' }));
}
