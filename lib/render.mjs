...
export function renderReport(a) {
  ...
  // 日次
  const dailyChart = stackedBars(
    a.daily.map((d) => ({ ...d, label: d.date })),
    [{ key: 'time', name: 'ビリヤード' }, { key: 'item', name: '物販' }, ...(k.gameYen > 0 ? [{ key: 'game', name: 'ゲーム' }] : [])],
    { lineKey: 'guests' },
  );
  ...
  <div class="report">
    ...
    ${section('日次売上の推移', '締めベース・棒=売上内訳／折れ線=客数', dailyLegend + dailyChart)}
