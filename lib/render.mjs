...
export function renderReport(a) {
  ...
  // 期間セレクタ（全期間／各月度）で KPI と日次推移を連動切替。既定は全期間。
  const multiMonth = a.months.length > 1;
  const activeP = 'all';
  const periods = [{ key: 'all', label: '全期間', kpi: k, days: a.daily }, ...a.months.map((m) => ({ key: m.key, label: m.label, kpi: m.kpi, days: m.days }))];
  const periodSel = `<div class="mtabs">${periods.map((p) => `<button type="button" class="monthbtn${p.key === activeP ? ' active' : ''}" data-m="${p.key}">${esc(p.label)}</button>`).join('')}</div>`;
  const kpiSection = multiMonth
    ? periodSel + periods.map((p) => `<div class="monthpane" data-m="${p.key}"${p.key === activeP ? '' : ' hidden'}>${kpiCards(p.kpi)}</div>`).join('')
    : kpiCards(k);
  // 日次推移（同じ data-m のパネルが上のセレクタで連動切替）
  ...
  const dailyBody = multiMonth
    ? dailyLegend + periods.map((p) => `<div class="monthpane" data-m="${p.key}"${p.key === activeP ? '' : ' hidden'}>${dailyChartOf(p.days)}</div>`).join('')
    : dailyLegend + dailyChartOf(a.daily);
  // 月次サマリー表（月度ごと＋合計）
  const sumRows = [...a.months.map((m) => ({ label: m.label, ...m.kpi })), { label: '合計', ...k }];
  const summaryTbl = `<div class="tblscroll"><table class="tbl mtbl"><thead><tr><th>月度</th><th>営業日</th><th>総売上</th><th>ビリヤード</th><th>物販</th><th>客数</th><th>客単価</th><th>平均日商</th><th>取消</th></tr></thead><tbody>${sumRows.map((r, i) => `<tr class="${i === sumRows.length - 1 ? 'totalrow' : ''}"><td>${esc(r.label)}</td><td>${r.days}</td><td class="em">${yen(r.totalYen)}</td><td>${yen(r.timeYen)}</td><td>${yen(r.itemYen)}</td><td>${r.guestCount}</td><td>${yen(r.perGuest)}</td><td>${yen(r.avgDaily)}</td><td>${r.voidCount}件</td></tr>`).join('')}</tbody></table></div>`;
  ...
  <div class="report">
    ...
    ${section('日次売上の推移', '締めベース・棒=売上内訳／折れ線=客数' + (multiMonth ? '・上のボタンで月度切替' : ''), dailyBody)}

    ${multiMonth ? section('月次サマリー', '締めベース・月度ごとの集計', summaryTbl) : ''}
