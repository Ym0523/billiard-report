...
function catDetailSection(scope, gran) {
  if (!scope.catKeysPresent.length) return section('物販カテゴリの詳細', '明細ベース', '<p class="note">物販の売上はありません。</p>');
  const series = scope.catKeysPresent.map((k) => ({ key: k, name: scope.catLabels[k] }));
  const lg = legend(series.map((s) => ({ label: s.name })));
  const rows = gran === 'month' ? scope.itemMonthlyByCat.map((d) => ({ ...d, x: d.label })) : scope.itemDailyByCat.map((d) => ({ ...d, x: (d.date || '').slice(5) }));
  const tbl = `<table class="tbl"><thead><tr><th>カテゴリ</th><th>売上</th><th>構成比</th><th>主要商品（点数）</th></tr></thead><tbody>${scope.categoryDetail.map((c) => `<tr><td>${esc(c.label)}</td><td class="em">${yen(c.yen)}</td><td>${pct(c.share)}</td><td class="prodcell">${c.top.map((p) => `${esc(p.name)}<span class="q">×${p.qty}</span>`).join('、') || '—'}</td></tr>`).join('')}</tbody></table>`;
  return section('物販カテゴリの詳細', (gran === 'month' ? 'カテゴリ別の月次推移' : 'カテゴリ別の日次推移') + '（積み上げ）＋主要商品・明細ベース', lg + stackedBars(rows, series, {}) + tbl);
}

function weekdaySection(scope) {
  return section('曜日別の平均日商', '締めベース', vBars(scope.weekday.map((w) => ({ label: w.wd, value: w.avgTotal })), { color: 'var(--c1)', kfmt: (v) => (v / 1000).toFixed(0) + 'k', vfmt: (v) => yen(v) }));
}
function hourlySection(scope) {
  return section('時間帯別の会計件数', '明細の会計時刻ベース', vBars(scope.hourly.map((h) => ({ label: h.h + '時', value: h.count })), { color: 'var(--c3)', vfmt: (v) => v + '件' }));
}
function productsSection(scope) {
  const rows = scope.products.map((p) => ({ label: p.name, value: p.yen, sub: `${p.qty}点` }));
  return section('商品ランキング Top12', '売上・明細ベース', rows.length ? hBars(rows, { labW: 170 }) : '<p class="note">物販の売上はありません。</p>');
}
...
function scopeBody(scope, gran, label) {
  ...
  return `
    ...
    ${catDetailSection(scope, gran)}
    <div class="grid2">${weekdaySection(scope)}${hourlySection(scope)}</div>
    <div class="grid2">${productsSection(scope)}${regularsSection(scope)}</div>
    <div class="grid2">${specialSection(scope)}${medalSection(scope)}</div>
    ${cashSection(scope)}`;
}
...
export function renderReport(a) {
  ...
  const modeTabs = `<div class="modetabs">
    <button type="button" class="modebtn${defMode === 'year' ? ' active' : ''}" data-mode="year">年度レポート</button>
  ...
  <div class="report">
    ...
    <div class="controls">${modeTabs}${yearSel}${monthSel}</div>

    ${panes}

    <footer class="rfoot">締め・明細から自動生成。取消(void)は売上から除外。金額は税込・円。上のボタンで年度／月次と期間を切替えられます。</footer>
  </div>`;
}
