...
f.addEventListener('submit',async(e)=>{
  ...
  try{
    ...
    // 月度切替ボタンの配線（暗号化本文は静的なので復号後にここで付ける）
    document.querySelectorAll('.mtabs').forEach(function(tabs){
      tabs.addEventListener('click',function(ev){
        var b=ev.target.closest('.monthbtn'); if(!b) return;
        var root=tabs.parentElement, m=b.getAttribute('data-m');
        root.querySelectorAll('.monthbtn').forEach(function(x){x.classList.toggle('active',x===b);});
        root.querySelectorAll('.monthpane').forEach(function(p){p.hidden=(p.getAttribute('data-m')!==m);});
      });
    });
  }catch(_){
