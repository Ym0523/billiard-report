...
f.addEventListener('submit',async(e)=>{
  ...
  try{
    ...
    // 期間セレクタの配線（暗号化本文は静的なので復号後にここで付ける）。
    // 同じ data-m を持つ全パネル（KPI・日次推移）をまとめて切替える。
    document.querySelectorAll('.mtabs').forEach(function(tabs){
      tabs.addEventListener('click',function(ev){
        var b=ev.target.closest('.monthbtn'); if(!b) return;
        var m=b.getAttribute('data-m');
        tabs.querySelectorAll('.monthbtn').forEach(function(x){x.classList.toggle('active',x===b);});
        document.querySelectorAll('.monthpane').forEach(function(p){p.hidden=(p.getAttribute('data-m')!==m);});
      });
    });
  }catch(_){
