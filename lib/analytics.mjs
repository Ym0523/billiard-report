...
export function analyze(store, meta = {}) {
  ...
  function computeScope(cls) {
    ...
    const prodMap = new Map(), catByDate = new Map(), catByMonth = new Map(), catProd = {};
    for (const s of sales) {
      const date = cidToDate.get(s.closingId), mk = date ? date.slice(0, 7) : null;
      for (const l of (s.lines || [])) {
        ...
        if (mk) { const d = catByMonth.get(mk) || {}; d[l.cat] = (d[l.cat] || 0) + gross; catByMonth.set(mk, d); }
    ...
    const catKeysPresent = CAT_ORDER.filter((k) => (catYen[k] || 0) > 0);
    const itemDailyByCat = cls.map((c) => { const row = { date: c.businessDate }; const d = catByDate.get(c.businessDate) || {}; catKeysPresent.forEach((k) => (row[k] = yenOf(d[k] || 0))); return row; });
    const itemMonthlyByCat = mKeys.map((mk) => { const row = { date: mk, label: `${+mk.slice(5, 7)}月` }; const d = catByMonth.get(mk) || {}; catKeysPresent.forEach((k) => (row[k] = yenOf(d[k] || 0))); return row; });
    const categoryDetail = categories.map((c) => ({ key: c.key, label: c.label, yen: c.yen, share: c.yen / (itemGross || 1), top: [...(catProd[c.key] || new Map()).entries()].map(([name, v]) => ({ name, qty: v.qty, yen: yenOf(v.yen) })).sort((a, b) => b.yen - a.yen).slice(0, 4) }));
    ...
    closed.forEach((r) => stamp(r.at, r.yen)); sales.forEach((r) => stamp(r.at, r.yen));
    const act = hours.filter((x) => x.count > 0);
    const hFrom = act.length ? act[0].h : 0, hTo = act.length ? act[act.length - 1].h : 23;
    const hourly = hours.slice(hFrom, hTo + 1).map((x) => ({ ...x, yen: yenOf(x.yen) }));
    ...
    return {
      kpi, daily, monthly, monthSummary, mix, categories, itemGross: yenOf(itemGross), products,
      catKeysPresent, catLabels: Object.fromEntries(CAT_ORDER.map((k) => [k, CAT_LABEL[k]])), itemDailyByCat, itemMonthlyByCat, categoryDetail,
  ...
  const years = [...byYear.keys()].sort().map((k) => ({ key: k, label: `${+k}年度`, scope: computeScope(byYear.get(k)) }));
