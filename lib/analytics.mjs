...
export function analyze(store, meta = {}) {
  ...
  // 月度（businessDate の年月）でグルーピング。日次推移の月切替＋月度KPI＋月次サマリー用。
  const months = [];
  closings.forEach((c, i) => {
    const key = c.businessDate.slice(0, 7);
    let m = months.find((x) => x.key === key);
    if (!m) { m = { key, label: `${+key.slice(0, 4)}年${+key.slice(5, 7)}月度`, days: [], cls: [] }; months.push(m); }
    m.days.push(daily[i]); m.cls.push(c);
  });
  months.forEach((m) => { m.kpi = kpiFrom(m.cls); delete m.cls; });

  // ---- カテゴリ別（物販の内訳・明細ベース）----
  const cidToDate = new Map(closings.map((c) => [c.id, c.businessDate]));
  const catYen = {}; CAT_ORDER.forEach((k) => (catYen[k] = 0));
  const prodMap = new Map();          // name -> {qty, yen}
  const catByDate = new Map();        // businessDate -> {cat: yen}
  const catProd = {};                 // cat -> Map(name -> {qty, yen})
  for (const s of sales) {
    ...
    for (const l of (s.lines || [])) {
      ...
      const cp = catProd[l.cat].get(l.name) || { qty: 0, yen: 0 };
      cp.qty += (l.qty || 1); cp.yen += gross; catProd[l.cat].set(l.name, cp);
    }
  }
  ...
  // 詳細：カテゴリ別の日次推移（積み上げ用）＋カテゴリごとの主要商品
  const catKeysPresent = CAT_ORDER.filter((k) => (catYen[k] || 0) > 0);
  const itemDailyByCat = closings.map((c) => {
    const row = { date: c.businessDate }; const d = catByDate.get(c.businessDate) || {};
    catKeysPresent.forEach((k) => (row[k] = yenOf(d[k] || 0))); return row;
  });
  const categoryDetail = categories.map((c) => ({
    key: c.key, label: c.label, yen: c.yen, share: c.yen / (itemGross || 1),
    top: [...(catProd[c.key] || new Map()).entries()].map(([name, v]) => ({ name, qty: v.qty, yen: yenOf(v.yen) }))
      .sort((a, b) => b.yen - a.yen).slice(0, 4),
  }));
  ...
  return {
    ...
    daily, months, mix, categories, itemGross: yenOf(itemGross), products, weekday, hourly,
    catKeysPresent, catLabels: Object.fromEntries(CAT_ORDER.map((k) => [k, CAT_LABEL[k]])), itemDailyByCat, categoryDetail,
