...
export function analyze(store, meta = {}) {
  ...
  function computeScope(cls) {
    ...
    // 時間帯別（営業時間の並びで固定：10時スタート〜翌4時終わり）
    ...
    const hourly = HOUR_ORDER.map((h) => ({ h, count: hours[h].count, yen: yenOf(hours[h].yen) }));
    ...
    return {
      kpi, daily, monthly, monthSummary, visitDaily, visitMonthly, visitTotal, mix, categories, itemGross: yenOf(itemGross), products, categoryDetail,
